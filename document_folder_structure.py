...
"""
Possible file names to truncate, editable.
"""
TRUNCATE_EXTS = [".md", ".txt"]
"""
Extensions to truncate, editable.
"""
TRUNCATE_LINES = 10      # For README, LICENSE, etc.
"""
Number of lines to show for truncated files.
"""
LOG_LAST_LINES = 10      # For .log files
"""
Number of lines to show for log files.
"""
 
# === SETUP LOGGING ===
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

def preview_csv(filepath, max_lines=10):
    """
...
def preview_excel(filepath, max_lines=10):
    """
...
def load_config(config_file):
    """
    Loads a JSON configuration file and returns it as a dict.
...
def should_truncate_file(fname):
    """
    Determines if a file should be truncated in preview, by name/extension.
    """
    base = os.path.splitext(os.path.basename(fname))[0].upper()
    ext = os.path.splitext(fname)[1].lower()
    for t in TRUNCATE_FILES:
        if base.startswith(t):
            return True
    if ext in TRUNCATE_EXTS:
        if base.startswith("README") or base.startswith("LICENSE") or base.startswith("CHANGELOG"):
            return True
    return False
...
def generate_content_document(
    files_list, output_file="all_files_content.md", git_info=None,
    max_log_lines=LOG_LAST_LINES, truncate_lines=TRUNCATE_LINES, base_docs_path=None,
    project_dir=None
):
    """
    ...
    Args:
        ...
        git_info (tuple): Git info.
        max_log_lines (int): For log truncation.
        truncate_lines (int): For truncation of other files.
    ...
    for file_path in ordered_files:
        ...
        if is_binary(abs_path):
            ...
            out_lines.extend(preview_csv(abs_path, max_lines=10))
            out_lines.append("\n")
        elif ext == ".xlsx":
            out_lines.append("_(Excel preview)_\n")
            out_lines.extend(preview_excel(abs_path, max_lines=10))
            out_lines.append("\n")
        elif ext == ".xls":
            ...
            try:
                ...
                if len(lines) > max_log_lines:
                    out_lines.append(f"_(Showing last {max_log_lines} lines)_\n")
                out_lines.append(f"```{language}\n")
                out_lines.extend(lines[-max_log_lines:])
                out_lines.append("```\n")
            except Exception as e:
                logging.warning(f"Could not read {abs_path}: {e}")
                out_lines.append(f"(Could not read file: {e})\n")
        elif should_truncate_file(abs_path):
            try:
                with open(abs_path, 'r', encoding='utf-8', errors='replace') as fin:
                    lines = fin.readlines()
                if len(lines) > truncate_lines:
                    out_lines.append(f"_(Showing first {truncate_lines} lines)_\n")
                out_lines.append(f"```{language}\n")
                out_lines.extend(lines[:truncate_lines])
                out_lines.append("```\n")
            except Exception as e:
                logging.warning(f"Could not read {abs_path}: {e}")
                out_lines.append(f"(Could not read file: {e})\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--truncate-lines", type=int, default=TRUNCATE_LINES, help="Max lines for files like README, LICENSE, etc.")
    parser.add_argument("--max-log-lines", type=int, default=LOG_LAST_LINES, help="Max lines to show for .log files")
    ...
    generate_content_document(
        ...
        truncate_lines=args.truncate_lines,
